...
Storage = require("__DedLib__/modules/templates/storage").new{
...
function Storage.PlayerUiOpen.add(player, entity)
	if entity and entity.valid then
		...
		local chests = Storage.global.playerUiOpen[player.index] or {}
...
function Storage.PlayerUiOpen.removeChest(entity)
	...
	for playerIndex, chests in pairs(Storage.global.playerUiOpen) do
		local oldLength = #chests
		local newChests = Table.filter(chests, function(chest) return chest ~= entity end)
		Storage.global.playerUiOpen[playerIndex] = newChests
...
function Storage.PlayerUiOpen.get(player)
	return Storage.global.playerUiOpen[player.index]
end
...
function Storage.PlayerFastReplace.add(player, replacementChestName, oldEntity)
	...
	Storage.global.playerFastReplace[player.index] = {replacementChestName=replacementChestName, requestFilters=requestFilters, storageFilter=storageFilter, requestFromBufferToggle=requestFromBufferToggle}
end
...
function Storage.PlayerFastReplace.remove(player)
	...
	Storage.global.playerFastReplace[player.index] = nil
end

function Storage.PlayerFastReplace.get(player)
	return Storage.global.playerFastReplace[player.index]
end
...
function Storage.PlayerFastReplaceEvents.add(player, position)
	...
	if not Storage.global.playerFastReplaceEvents[player.index] then
		Storage.global.playerFastReplaceEvents[player.index] = {}
	end
	
	Storage.global.playerFastReplaceEvents[player.index][absolutePosition] = game.tick
end
...
function Storage.PlayerFastReplaceEvents.get(player, position)
	...
	local playerData = Storage.global.playerFastReplaceEvents[player.index]
	if playerData then
		local absolutePosition = Storage.getAbsolutePosition(player.surface, position)
		return Storage.global.playerFastReplaceEvents[player.index][absolutePosition] or game.tick
	end
	return game.tick
end
...
function Storage.PlayerFastReplaceEvents.purge()
	...
	for playerIndex, positions in pairs(Storage.global.playerFastReplaceEvents) do
